...
func debug_print_database():
	print("")
	print("=== OPEN WORLD DATABASE EXPORT ===")
	print("Total Nodes: ", open_world_database.node_monitor.node_data_lookup.size())
	
	var top_level_nodes = open_world_database.get_top_level_nodes()
	top_level_nodes.sort_custom(func(a, b): return a.uid < b.uid)
	
	for node_data in top_level_nodes:
		_debug_output_node_recursive(node_data, 0)
	print("=== END DATABASE EXPORT ===")
	
	_debug_output_chunked_structure()

func _debug_output_chunked_structure():
	print("")
	print("=== CHUNKED STORAGE STRUCTURE ===")
	
	var sorted_sizes = open_world_database.node_data_lookup_chunked.keys()
	sorted_sizes.sort()
	
	for size_enum in sorted_sizes:
		print("Chunk Size: ", size_enum)
		
		var sorted_positions = open_world_database.node_data_lookup_chunked[size_enum].keys()
		...
		for chunk_pos in sorted_positions:
			...
			var nodes_in_chunk = open_world_database.node_data_lookup_chunked[size_enum][chunk_pos]
			...
			for node_data in sorted_nodes:
				_debug_output_chunked_node_recursive(node_data, 4)
		print("")
	
	print("=== END CHUNKED STORAGE STRUCTURE ===")

func _debug_output_chunked_node_recursive(node_data: NodeData, indent_level: int):
	...
	if node_data.children.size() > 0:
		...
		for child in sorted_children:
			_debug_output_chunked_node_recursive(child, indent_level + 4)
	
	print("")

func _debug_output_node_recursive(node_data: NodeData, depth: int):
	var indent = "\t".repeat(depth)
	var size_name = str(open_world_database.get_size_enum(node_data.size))
	var chunk = open_world_database.get_chunk_position(node_data.position)
	...
	if node_data.children.size() > 0:
		...
		for child in sorted_children:
			_debug_output_node_recursive(child, depth + 1)
